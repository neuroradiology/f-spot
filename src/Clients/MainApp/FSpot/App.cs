...
namespace FSpot
{
    public class App : Unique.App
    {
        ...
        App () : base("org.gnome.FSpot.Core", null, "Import", Command.Import, "View", Command.View, "Organize", Command.Organize, "Shutdown", Command.Shutdown,
        ...
        void HandleSlideshow (string tagname)
        {
            ...
            IPhoto[] photos;
            if (tag != null)
                photos = Database.Photos.Query (new Tag[] { tag }); else if (Preferences.Get<int> (Preferences.SCREENSAVER_TAG) == 0) {
            ...
            if (photos.Length > 0) {
                ...
                hbox.PackStart (new Gtk.Image (Gtk.Stock.DialogWarning, Gtk.IconSize.Dialog), false, false, 0);
                outer.PackStart (new Gtk.Label (String.Empty));
